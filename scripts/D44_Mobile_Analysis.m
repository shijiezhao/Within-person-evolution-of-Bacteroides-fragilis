...
fid1=fopen('MobileSequences.fasta','w');
fid2=fopen('MobileElementPattern.csv','w');
fprintf(fid2,['MobileElement'])
% 1.1 Write the first row of the pattern file
fprintf(fid2,[',' 'Type' ',' 'Coverage' ',' 'Region Length' ])
...
%                 fprintf(fid2,[',' num2str(0)]);
            ...
            X=[X str2num(AllSampNames{j}(6:9))];
            Y=[Y coverage];
            if find(Z(2,:)==j);
                Z(3,find(Z(2,:)==j)) = coverage;
            end
%         end
    end
    ...
    if MC>-1;
        ...
        fprintf(fid1,['>Region No.' num2str(is) '\t' 'Chromosome:' num2str(loc1(1)) ':' num2str(loc1(2)) '-' num2str(loc2(2)) '\t' 'Type' num2str(whichtype) '\t' 'Coverage:' num2str(MC) '\t' 'length of the chr:' num2str(length(fr(loc1(1)).Sequence)) '\n' SeqtoWrite '\n'])
        fprintf(fid2,[',' 'Type' num2str(whichtype) ',' num2str(MC) ',' num2str(loc2(2)-loc1(2))])
        for metype = 1:length(type);
            fprintf(fid2,[',' num2str(type(metype))]);
        end
        saveas(gcf,['CoverageMap/' 'Type' num2str(whichtype) '_Region_No.' num2str(is)  '.pdf'],'pdf'); 
        fprintf(fid2,['\n']);
    end
%      pause
    ...
    figure(1);hold on; 
    plot(X,Y,'O');
    saveas(gcf,['CoverageMap/' 'TimeSeries_Type' num2str(whichtype) '_Region_No.' num2str(is)  '.pdf'],'pdf'); 
    
    close all force;
    figure(1);hold on; 
    plot(Z(1,:),Z(3,:),'O');
    saveas(gcf,['CoverageMap/' 'Abundance_' num2str(whichtype) '_Region_No.' num2str(is)  '.pdf'],'pdf'); 
end