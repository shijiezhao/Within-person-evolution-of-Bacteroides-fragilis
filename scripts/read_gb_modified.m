...
for i=1:length(ScafNames)
    ...
    if exist(gbfilename, 'file')
        ...
        if ~isfield(genbankFile,'Sequence')
            error('Check that the length of the header of the genbank file is at least 79 characters including whitespaces');
        end
    ...
    CDS{i} = parse_all_locations_gb(genes, scafSeq) ;  %also reverses strands in this, sorts by position
