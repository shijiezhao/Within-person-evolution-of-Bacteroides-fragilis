...
for i=1:numel(RefGenomes)
    ...
    if ~exist([ref_folder '/Reference_Genomes/' RefGenomes{i} '/genome_bowtie2.1.bt2'],'file')
        ...
        c={}; c{end+1}=['bowtie2-build -q genome.fasta genome_bowtie2'];
...
for i = 1:length(SampleTable)
    ...
    if (~(exist(tname_out1,'file'))  | overwrite) & ~(exist('sickle2050/pairedendtb/','dir') | exist('sickle2050/pairedendecoli/','dir'))
        trim_cmds{end+1} = ['cutadapt -a ' adapter ' ' tname_in1 ' > ' tname_out1];
        trim_dirs{end+1} = [s.Sample '/' adapter_dir];
    end
    
    if paired & (~(exist(tname_out2,'file')) | overwrite) & ~(exist('sickle2050/pairedendtb/','dir') | exist('sickle2050/pairedendecoli/','dir'))
...
for i=1:length(SampleTable)
    ...
    for f=fi(:)'
        ...
        if paired %SK
            ...
            if (~(exist(fname_out1,'file') && exist(fname_out2,'file')) || overwrite) & ~(exist('sickle2050/pairedendtb/','dir') | exist('sickle2050/pairedendecoli/','dir'))
                % no filtering
        ...
        if ~exist([dr '/aligned.sam']) & ~exist([dr '/aligned.sorted.bam'])
            ...
            switch ae.Method
                ...
                case 'bowtie2pairedfilter' % allows no ambiguous characters
                    if Phred_offset == 64
                        cmds{end+1} = ['bowtie2 -X 2000 --no-mixed --very-sensitive --n-ceil 0,0.01 --phred64 -x  genome_bowtie2' ...
                            '-1  ' drf '/filter_reads_1.fastq -2  ' drf '/filter_reads_2.fastq -S ' dra '/aligned.sam'];
                    else
                        cmds{end+1} = ['bowtie2 -X 2000 --no-mixed --very-sensitive --n-ceil 0,0.01 -x  genome_bowtie2' ...
                ...
                case 'bowtie2pairedxt' % allows no ambiguous characters
                    if Phred_offset == 64
                        cmds{end+1} = ['bowtie2 -X 2000 --very-sensitive --n-ceil 0,0.01  --phred64 -x  genome_bowtie2' ... 
                            '-1 ' drf '/filter_reads_1.fastq -2 ' drf '/filter_reads_2.fastq -S ' dra '/aligned.sam'];
                    else
                        cmds{end+1} = ['bowtie2 -X 2000--very-sensitive --n-ceil 0,0.01 -x  genome_bowtie2' ...
                ...
                case 'bowtie2discordant' % allows no ambiguous characters
                    if Phred_offset == 64
                        cmds{end+1} = ['bowtie2 -X 2000 --dovetail --very-sensitive --n-ceil 0,0.01 --phred64 -x  genome_bowtie2' ...
                            '-1 ' drf '/filter_reads_1.fastq -2 ' drf '/filter_reads_2.fastq -S ' dra '/aligned.sam'];
                    else
                        cmds{end+1} = ['bowtie2 -X 2000 --dovetail --very-sensitive --n-ceil 0,0.01  -x  genome_bowtie2' ...
                            ' -1 ' drf  '/filter_reads_1.fastq -2 ' drf '/filter_reads_2.fastq -S ' dra '/aligned.sam '];
                    end
...
for i=1:length(all_dirs)
    if ~exist([all_dirs{i} '/strain.pileup'],'file') & ~exist([all_dirs{i} '/diversity.mat'],'file') ; %remove pileup2
        ...
        if Phred_offset==64
            cmds{end+1} = ['samtools mpileup -q30 -6 -O -s -x -d3000 -f  "' ref_folder '/Reference_Genomes/' all_genomes{i} '/genome.fasta" aligned.sorted.bam > strain.pileup'] ;
        else
            cmds{end+1} = ['samtools mpileup -q30 -s -O -x -d3000 -f "' ref_folder '/Reference_Genomes/' all_genomes{i} '/genome.fasta" aligned.sorted.bam > strain.pileup'] ;
        end
        dirs{end+1} = all_dirs{i} ;
    end
end
...
for i=1:length(all_dirs)
    if (~exist([all_dirs{i} '/strain.vcf'],'file')) | (~exist([all_dirs{i} '/variant.vcf'],'file')) ;
        ...
        if Phred_offset==64
            cmds{end+1} = ['samtools mpileup -q30 -6 -S -d3000 -ugf "' ref_folder '/Reference_Genomes/' all_genomes{i} '/genome.fasta" aligned.sorted.bam > strain; bcftools call -c strain > strain.vcf ; bcftools call -cv strain > variant.vcf; rm strain'] ;
        else
            cmds{end+1} = ['samtools mpileup -q30 -S -d3000 -ugf"' ref_folder '/Reference_Genomes/' all_genomes{i} '/genome.fasta" aligned.sorted.bam > strain; bcftools call -c strain > strain.vcf ; bcftools call -cv strain > variant.vcf; rm strain'] ;
        end
...
run_parallel_unix_commands_fast({['matlab -r "path(' char(39) '/groups/kishony/illumina_pipeline_tami/' char(39) ',path); pileup_to_diversity_matrix"']},long_q,Parallel,dirs);
...
run_parallel_unix_commands_fast({'samtools index aligned.sorted.bam; rm strain.pileup; rm -r ../../cutadapted; rm ../*.fastq; rm ../../*.fastq'},mini_q,Parallel,dirs);
