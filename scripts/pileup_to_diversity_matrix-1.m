...
[ChrStarts,GenomeLength,~,ScafNames]=genomestats(REF_GENOME_DIRECTORY); %RUN_ON_CLUSTER
fprintf(1,['Numscafs: ' num2str(numel(ScafNames)) '\n']);
fprintf(1,['GenomeLength: ' num2str(GenomeLength) '\n']);
...
while ischar(line)
    ...
    %fprintf(1,[l{2} ' \n']);
    %fprintf(1,[l{1} ' \n']);
    %fprintf(1,[ScafNames{1} ' \n']);
    %fprintf(1,[num2str(chr) ' ' num2str(position_on_chr) ' \n']);
        
    ref=find(nts==int16(l{3}));
    if ref > 4 %if lowercase, make uppercase
        ref = ref-4;
    end
    calls=int16(l{5});
